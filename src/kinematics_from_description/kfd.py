...
class KinematicsFromDescriptionTool():
    ...
    def load_desc_from_param(self):
        ...
        server
        """
        key = f'{self.robot_namespace}/robot_description'
        try:
            self.robot = URDF.from_parameter_server(
                key=key)
        except KeyError as ke:
            print((f"Error: {key} not found on the ROS parameter server. "
    ...
    def _populate(self):
        ...
        self.body_frame = self._search(
            self.links,
            f'{self.robot_namespace}/{self.body_frame_name}',
            'name')

        self.space_frame = self._search(
            self.links,
            f'{self.robot_namespace}/{self.space_frame_name}',
            'name')

        if self.body_frame==None or self.space_frame==None:
            raise ValueError()
    ...
    def _calc_Slist(self):
        ...
        """
        num_axes = len(
            self.axis_list)  # number of revolute axes defines Slist shape
        Slist = np.zeros((6, num_axes))
        tf = np.eye(4, dtype='float64')
        joint = self.rev_joint_list[0]
    ...
    def _build_link_lists(self):
        ...
        current_SE3_to_base = np.eye(4, dtype='float64')

        # iterate backwards through link/joint tree up to base
        while (current_link != None and
                (current_link.name != self.space_frame.name)):
    ...
    def _get_joint_translation(self, joint):
        ...
        """
        if joint.origin == None or joint.origin.xyz == None:
    ...
    def _get_joint_rotation(self, joint):
        ...
        """
        if joint.origin == None or joint.origin.rpy == None:
    ...
    def _search(self, list_, key, attr):
        ...
        """
        result = [element for element in list_ if getattr(
            element, attr) == key]
        if len(result) > 0:
            return result[0]
        else:
